...
class BatallaNaval:
    ...
    def __choose_position(self, ship_size=0):
        while True:
            try:
                row = int(
                    input(f'Ingrese una fila del 0 al {self.__board_len - 1}: '))
                column = int(
                    input(f'Ingrese una columna del 0 al {self.__board_len - 1}: '))
    ...
    def __get_position(self, position_quantity):
        ...
        while True:
            orientation = input(
                'Ingrese alineación (H: Horizontal, V: Vertical): ').strip().upper()
    ...
    def __attack(self, player):
        print(
            f'Seleccione posiciones entre 0 y {self.__board_len} para atacar')
    ...
    def __cpu_attack(self):
        while True:
            row = random.randint(0, self.__board_len - 1)
            column = random.randint(0, self.__board_len - 1)
            # O could be a hit or miss marker
            if self.__players[0]['board'][row][column] not in ['X', 'O']:
    ...
    def init_game(self):
        print(
            f'Creando un tablero de {self.__board_len} por {self.__board_len}\n')
        self.__assign_piece()
        # Generar el tablero del cpu
        self.__players[1]['board'] = self.__generate_opponent_board()
        ...
        for ship, info in self.__ships.items():
            ...
            while True:
                print(
                    f'Coloca el barco "{ship.upper()}" en tu tablero (ocupara {info["posiciones"]} posiciones)')
                ...
                if self.__can_place_ship(self.__players[0]['board'], positions):
                    ...
                    print(
                        f'El  "{ship.upper()}" no puede colocarse en ese lugar')

        print("Tablero Final:")
        for fila in self.__players[0]['board']:
            print(fila)

        # deep copy for ships dictionary
        self.__players[0]['ships'] = {k: v.copy()
                                      for k, v in self.__ships.items()}
        # deep copy for ships dictionary
        self.__players[1]['ships'] = {k: v.copy()
                                      for k, v in self.__ships.items()}
        ...
        while self.__ongoing_game(self.__players[self.__current_turn]['opponent']):
            print(
                f'Es el turno del jugador {self.__players[self.__current_turn]["name"]}')
        ...
        print(
            f'El {self.__players[self.__current_turn]["name"]} es el ganador')
